        ...
        <CardContent className="pt-4">
          <div className="h-[60vh] overflow-y-auto space-y-3 mb-4 pr-2">
            {mensajes.map(m => (
              <div key={m.id} className={cn("flex gap-2", m.tipo === "sistema" && "justify-center")}>
                {m.tipo === "sistema" ? (
                  <div className="bg-gray-100 text-gray-600 text-xs px-3 py-1.5 rounded-full flex items-center gap-1">
                  ...
                  <div className={cn("flex gap-2 max-w-[80%]", m.user_pena_id === userPena?.id && "ml-auto flex-row-reverse")}>
                    ...
                    <div>
                      {m.user_pena_id !== userPena?.id && <p className="text-xs text-gray-500 mb-0.5">{m.autor?.nombre_completo || "Anónimo"}</p>}
                      <div className={cn("rounded-2xl px-4 py-2 text-sm", m.user_pena_id === userPena?.id ? "bg-indigo-500 text-white rounded-tr-none" : "bg-gray-100 text-gray-800 rounded-tl-none")}>
                        {m.mensaje}
                      </div>
                    </div>
                  </div>
          ...
          <div className="flex gap-2">
            <input value={texto} onChange={e => setTexto(e.target.value)} onKeyDown={e => e.key === "Enter" && enviar()}
              placeholder="Escribe un mensaje..." className="flex-1 px-4 py-2 border rounded-lg text-sm" />
